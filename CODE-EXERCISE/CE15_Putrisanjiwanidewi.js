...
function duckShoot(ammo, aim, ducks) {
    ...
    let newDucks = ducks.split("");
    const totalShoot = Math.floor(ammo * aim);
    for (let i = 0; i < totalShoot; i++) {
        let idx = newDucks.indexOf("2");
        newDucks.splice(idx, 1, "X");
    }

    return newDucks.join("");
}
